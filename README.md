...
2. 按这个顺序找 DSH：
   - `~/Library/Application Support/DeepSeek Harness/runtime` 里已安装的副本
   - PATH 上的 `dsh`
   - 全局 npm 包 `@deepseek-ai/dsh`
3. 若都没有，则用 npm 把 `@deepseek-ai/dsh` 装到上述 Application Support 目录
4. 执行 `node --expose-internals <入口> web --host 127.0.0.1 --port 0`
5. 从进程输出解析 `dsh web: http://127.0.0.1:<端口>`，在窗口中打开
6. 退出时结束 dsh 进程组

Application Support 里一旦有副本，就会优先用它，不再走 PATH 上的 `dsh`。
...
- macOS 26.0 或更高版本
- Node.js 22+（本机已安装即可；从 Dock 启动时应用会自己补上 Homebrew / nvm 等 PATH）
...
菜单 **DeepSeek Harness → 检查 DSH 更新…**（⇧⌘U）会查询 npm registry（优先用本机 `npm config get registry`，否则 npmmirror / npmjs）。这里更新的是本地 `@deepseek-ai/dsh`，不是桌面应用本身。

启动后也会在后台检查；有新版本时弹出系统对话框。点 **更新** 会装到 Application Support 并重启本地服务，不必重装本应用。点 **稍后** 会跳过该版本。运行中每 6 小时再查一次。

同菜单里还有 **打开日志**，会在 Finder 中显示日志文件。
